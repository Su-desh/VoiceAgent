...
class GeminiLiveBridge:
    ...
    async def send_audio_chunk(self, base64_pcm: str) -> bool:
        if not self.is_connected or not self.session:
            return False
        try:
    ...
    async def _keepalive_loop(self):
        """Sends a 320-byte silent PCM frame every 12 seconds to prevent idle session timeout."""
        silent_frame = b"\x00" * 320
        try:
            while not self._is_closing and self.session:
                await asyncio.sleep(12)
                if self.is_connected and self.session and not self._is_closing:
                    try:
                        await self.session.send_realtime_input(
                            media=types.Blob(data=silent_frame, mime_type="audio/pcm;rate=16000")
                        )
                    except Exception as e:
    ...
    async def _listen_to_gemini(self):
        full_transcript = []
        try:
            # Persistent multi-turn loop: each turn iteration consumes session.receive() until turn_complete
            while not self._is_closing and self.session:
                try:
                    async for response in self.session.receive():
                        ...
                        if sc:
                            ...
                            if sc.turn_complete:
                                ...
                                # Fail-safe: if Gemini Live generated 0 audio chunks and empty text, fallback to chat_service
                                if self.audio_chunks_in_turn == 0 and not final_text and self.last_user_prompt:
                                    logger.warning(f"Turn complete received with 0 audio chunks. Activating fallback for prompt: '{self.last_user_prompt}'")
                                    try:
                                        fallback_res = await chat_service.handle_message(
                                            message=self.last_user_prompt,
                                            session_id=self.session_id
                                        )
                                        await self.on_client_message({
                                            "type": "AGENT_RESPONSE",
                                            "text": fallback_res["reply"],
                                            "ui_events": fallback_res["ui_events"],
                                            "cart": fallback_res["cart"]
                                        })
                                    except Exception as fb_err:
                                        logger.error(f"Fallback execution error: {fb_err}")
                                else:
                                    await self.on_client_message({
                                        "type": "TURN_COMPLETE",
                                        "text": final_text
                                    })
                                # Exit inner receive generator so next turn re-enters session.receive() cleanly
                                break
                except asyncio.CancelledError:
                    break
                    ...
                    if self._is_closing:
                        break
                    logger.warning(f"Exception in live receive iteration: {inner_e}")
